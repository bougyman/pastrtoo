...
# {{{ Public Methods, these are what we publish
  ...
  def authenticate_filter(stem, channel, sender, command, msg, opts)
    ...
    return true if sender[:nick].match(/^(?:deww|deryl|leftyfb-?|darix|weigon|jvaughn|korozion|pgpkeys|napta|trey|icy|Pistos|tj|bougyman|manveru|thedonvaughn|Death_Syn|kez)$/i)
